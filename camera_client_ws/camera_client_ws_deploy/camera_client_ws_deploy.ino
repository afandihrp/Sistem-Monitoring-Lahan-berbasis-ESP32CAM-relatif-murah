...
void setup() {
  ...
  if(psramFound())
  {
    ...
    wsPacketBuffer = (uint8_t*)ps_malloc(1030);
    if (wsPacketBuffer) {
  ...
  else 
  {
    ...
    wsPacketBuffer = (uint8_t*)malloc(1030);
    if (wsPacketBuffer) {
  ...
  // ESP32 Core 3.x menggunakan ledcAttach and ledcWrite berdasarkan PIN.
...
bool sendFrameChunks(camera_fb_t * fb, bool useUdp) {
  ...
  uint16_t totalChunks = (totalLen + maxChunkSize - 1) / maxChunkSize;
  ...
  for (uint16_t chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
    ...
    if (useUdp) {
      ...
      // Write Big-Endian 6-byte header: [frameId (2B), totalChunks (2B), chunkIdx (2B)]
      uint8_t header[6];
      header[0] = (frameId >> 8) & 0xFF;
      header[1] = frameId & 0xFF;
      header[2] = (totalChunks >> 8) & 0xFF;
      header[3] = totalChunks & 0xFF;
      header[4] = (chunkIdx >> 8) & 0xFF;
      header[5] = chunkIdx & 0xFF;
      udp.write(header, 6);
      ...
      uint8_t stackPacket[1030];
      if (!packet) {
        packet = stackPacket;
      }

      // Write Big-Endian 6-byte header
      packet[0] = (frameId >> 8) & 0xFF;
      packet[1] = frameId & 0xFF;
      packet[2] = (totalChunks >> 8) & 0xFF;
      packet[3] = totalChunks & 0xFF;
      packet[4] = (chunkIdx >> 8) & 0xFF;
      packet[5] = chunkIdx & 0xFF;

      memcpy(packet + 6, fb->buf + offset, chunkSize);

      if (!sendWsBinary(packet, 6 + chunkSize, "stream frame")) {
        frameSentOk = false;
        break;
      }
    }
  ...
  if (!frameSentOk) {
    Serial.printf("[%s] Frame send failed!\n", useUdp ? "UDP" : "WS");
  }
  return frameSentOk;
}
...
void loop() {
  ...
  // Debug: Print pin states every 2 seconds (Removed for deployment to prevent CPU stall)
  /*
  ...
  */
  ...
  if (isConnected && WiFi.status() == WL_CONNECTED) {    
    ...
    if (useUdp) {
      ...
      if (udpInitialized) {
        sent = sendFrameChunks(fb, true);
      }
    }

    if (!sent) {
      sendFrameChunks(fb, false);
    }
    esp_camera_fb_return(fb);
  }
}
