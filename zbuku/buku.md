...
Sistem monitoring pada sektor pertanian dan peternakan merupakan
instrumen krusial untuk meminimalkan risiko kerugian finansial akibat
tindakan pencurian atau intrusi liar. Saat ini, solusi pengawasan yang
umum diterapkan di lapangan didominasi oleh perangkat CCTV
(*Closed-Circuit Television*) berbasis IoT (*Internet of Things*)
komersial. Namun, efektivitas operasional dari sistem komersial ini
sering terganggu oleh tingginya frekuensi alarm palsu (*false positive*)
dalam mengidentifikasi ancaman manusia. Hal tersebut terjadi karena
algoritma bawaan sistem umumnya belum mampu membedakan secara presisi
antara pergerakan intrusi nyata dengan gangguan lingkungan alami,
seperti pergerakan dedaunan, perubahan intensitas cahaya, atau aktivitas
...
pemantauan kustom yang mengintegrasikan kamera standar (*webcam*), SBC
(*Single-Board Computer*) sebagai papan pengontrol mikrokomputer lokal,
dan algoritma deteksi objek berbasis kecerdasan buatan seperti YOLO
(*You Only Look Once*). Kendati menawarkan fleksibilitas pemrograman
yang lebih tinggi, pendekatan kustomisasi konvensional ini dihadapkan
pada hambatan finansial berupa tingginya biaya investasi awal untuk
pengadaan unit pemroses berspesifikasi tinggi. Selain itu, untuk
menjalankan model deteksi objek yang kompleks secara real-time,
arsitektur sistem kustom ini kerap bergantung pada sewa layanan VPS
(*Virtual Private Server*) berbasis cloud untuk komputasi deteksi
...
pemantauan (*underwhelming cost-to-coverage ratio*). Mengingat lahan
agraris memiliki karakteristik area terbuka yang sangat luas, penggunaan
kamera dengan sudut pandang statis memaksa pengguna melipatgandakan
jumlah perangkat demi memperoleh cakupan pengawasan yang komprehensif.
Akibatnya, biaya pengadaan dan pemeliharaan sistem membengkak secara
eksponensial. Ditambah lagi, ketergantungan sistem pengawasan pada
konektivitas internet aktif dan server eksternal menjadi kendala kritis
bagi wilayah rural yang memiliki keterbatasan aksesibilitas jaringan
telekomunikasi. Hal ini membuat operasional sistem monitoring menjadi
tidak andal (*unreliable*) untuk berfungsi secara konsisten tanpa
gangguan transmisi data bagi pelaku usaha sektor agraris skala UMKM
(Usaha Mikro, Kecil, dan Menengah).

Oleh karena itu, diperlukan sebuah alternatif perancangan sistem
...
integrasi dengan beberapa unit kamera dedikasi terpisah secara skalabel
yang dapat ditambahkan sesuai kebutuhan
...
listrik tanpa koneksi internet kabel yang stabil, sehingga penggunaan
sistem keamanan berbasis *cloud* tidak memungkinkan.

Berdasarkan keterangan dari pemilik lahan, dibutuhkan sistem yang mampu:

- Memberikan notifikasi visual (foto) secara *real-time* melalui
  aplikasi Telegram saat terdeteksi penyusup.

- Memungkinkan kontrol kamera jarak jauh tanpa harus berlangganan
  layanan *cloud*.

- Beroperasi secara mandiri dengan biaya operasional rendah.

Dengan demikian, inti dari masalah pada judul *“Tingginya Biaya
Operasional pada Monitoring Lahan Peternakan Ayam”* adalah kebutuhan
akan sistem keamanan alternatif yang hemat biaya, tidak bergantung pada
internet permanen, namun tetap responsif dan fungsional dalam mendeteksi
ancaman keamanan.

## 1.2 Analisa Masalah

Masalah yang diangkat akan menyangkut multi-aspek. Diantaranya adalah
teknis, ekonomi, keamanan dan fungsionalitas, dan sosial. Berdasarkan
aspek yang sudah dijabarkan, berikut adalah penjelasan setiap aspek yang
terpengaruh.

### 1.2.1 Aspek Teknis:

- Efisiensi *Bandwidth* dan Kemandirian Jaringan: Hambatan utama sistem
  pemantauan konvensional adalah ketergantungan pada transmisi video
  langsung berkapasitas besar ke server eksternal
  yang boros bandwidth. Di wilayah kabupaten dekat kota dengan
  kestabilan jaringan yang sering berfluktuasi, hal ini menjadi tidak
  praktis dan kurang andal. Oleh karena itu, arsitektur komunikasi
  sistem ini dirancang mandiri menggunakan jaringan lokal (intranet).
  Seluruh lalu lintas data visual dari kamera ke unit pemroses (edge
  computing) diisolasi secara lokal melalui access point internal.
  Koneksi internet luar hanya digunakan secara minimal untuk mengirimkan
  notifikasi teks ringan via aplikasi pesan instan saat terdeteksi
  intrusi, sehingga konsumsi bandwidth eksternal dapat ditekan hingga
  hampir 0% (nol persen). Pendekatan ini memastikan sistem tetap
  beroperasi andal di area pinggiran kota (suburban) tanpa membutuhkan
...
- Keterbatasan Pemantauan Pasif secara Kontinu: Sistem pengawasan
  konvensional umumnya sangat bergantung pada interaksi aktif pengguna
...
- Keterbatasan Luas Cakupan Pengawasan (Sudut Pandang Statis): Perangkat
  kamera pengawas standar umumnya bekerja dengan sudut pandang (field of
  view) yang statis dan kaku. Dari sisi fungsionalitas, keterbatasan
  kontrol gerak ini menyebabkan adanya area titik buta (blind spot) yang
  luas pada lahan terbuka. Hal ini memaksa pengguna untuk
  melipatgandakan jumlah perangkat kamera jika ingin mengawasi seluruh
  area lahan secara komprehensif, sehingga menurunkan efisiensi
  fungsionalitas alat dalam memantau area pertanian atau peternakan yang
  luas secara dinamis.
...
memiliki urgensi, relevansi, dan nilai kemanfaatan yang sangat tinggi.
Kesimpulan dari analisis urgensi tersebut dapat dirangkum ke dalam
beberapa poin krusial berikut:

1.  **Urgensi Pengamanan Lahan Berbiaya Efisien:** Terdapat kebutuhan
    mendesak dari pelaku usaha sektor agraris (pertanian dan peternakan)
    skala UMKM untuk mengamankan properti mereka dari risiko pencurian
    dan intrusi liar. Lokasi lahan yang berada di wilayah kabupaten
    dekat kota memiliki tantangan infrastruktur telekomunikasi berupa
    kestabilan sinyal yang sering berfluktuasi, sehingga tidak dapat
    ditangani secara optimal oleh solusi pemantauan standar yang
    bergantung pada koneksi internet publik.

2.  **Kompleksitas Rancangan Multidimensional:** Permasalahan ini
...
4.  **Penyediaan Alternatif Sistem Berdaya Guna Tinggi:** Kehadiran
    ...
    fungsionalitas tinggi menjadi solusi krusial bagi pelaku usaha UMKM
    agraris. Dengan menekan biaya investasi awal dan memangkas biaya
